...
class TestFetchMoviesFeed:
    ...
    def test_feed_returns_hydrated_movie(
        ...
        fm = MagicMock()
        fm.get_next_movie = AsyncMock(return_value=_make_movie_details())
        _override_feed_manager(fm)

        try:
            resp = client.get("/api/v1/movies/feed", headers=AUTH_HEADERS)
        finally:
        ...
        assert data["movie_db_id"] == 1
        fm.get_next_movie.assert_awaited_once()
    ...
    def test_feed_returns_404_when_no_movies(
        self, mock_get_user, mock_prefs, mock_inc, mock_exc, client
    ):
        mock_get_user.return_value = FAKE_USER
        mock_prefs.return_value = _FAKE_PREF
    ...
    def test_feed_returns_404_when_user_not_found(self, mock_get_user, client):
        mock_get_user.return_value = None

        # Still need to override the feed_manager dep so FastAPI doesn't
        # try to instantiate the real one.
        fm = MagicMock()
        ...
        assert resp.status_code == 404
        fm.get_next_movie.assert_not_called()
