...
def test_item_cf_evaluator_respects_positive_relevance_threshold(tmp_path):
    train_df = pd.DataFrame(
        [(1, 10, 1.0)],
        columns=["user_id", "movie_id", "preference"],
    )
    ...
    similarity = csr_matrix(np.array([[0.0, 0.9], [0.9, 0.0]], dtype=np.float32))
    save_npz(tmp_path / "item_cf_train_matrix.npz", train_matrix)
    save_npz(tmp_path / "item_cf_similarity.npz", similarity)
    ...
    config.models.item_cf.relevance_preference_threshold = 0.0
    run(config)

    with open(tmp_path / "item_cf_metrics.json") as file_obj:
        report = json.load(file_obj)

    assert report["num_users_evaluated"] == 0
    assert report["metrics"]["precision@10"] == 0.0
