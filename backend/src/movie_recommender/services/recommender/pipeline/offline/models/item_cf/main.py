...
class ItemCFPipeline(RecommenderPipeline):
    ...
    def run_pipeline(self) -> None:
        ...
        print("\nStep 8: Building sparse matrix for Item-CF...")
        matrix.run(config)

        print("\nStep 9: Training item-item similarity model...")
        train_item_cf.run(config)

        print("\nStep 10: Evaluating Item-CF model...")
        metrics.run(config)
