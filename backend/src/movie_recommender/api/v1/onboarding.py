...
async def search_onboarding_movies(
    ...
    try:
        response = await _tmdb_fetcher._client.get(
...
async def _hydrate_tmdb_movie(db: AsyncSession, tmdb_id: int) -> int | None:
    ...
    try:
        detail_res = (
            await _tmdb_fetcher._client.get(
    ...
    # Insert stub row to get a DB ID
    ...
    movie_details = MovieDetails(
        ...
        trailer_url=_tmdb_fetcher._extract_trailer_url(detail_res),
        cast=_tmdb_fetcher._extract_cast_and_crew(detail_res),
        movie_providers=_tmdb_fetcher._extract_providers(detail_res),
        keywords=_tmdb_fetcher._extract_keywords(detail_res),
        collection=_tmdb_fetcher._extract_collection(detail_res),
        production_companies=_tmdb_fetcher._extract_production_companies(detail_res),
...
async def complete_onboarding(
    ...
    # Collect all DB movie IDs for vector computation
    all_movie_db_ids: list[int] = list(submission.grid_movie_ids)
    ...
    for tmdb_id in submission.search_movie_tmdb_ids:
        ...
        if existing:
            all_movie_db_ids.append(existing.id)
        else:
            new_db_id = await _hydrate_tmdb_movie(db, tmdb_id)
            if new_db_id is not None:
                all_movie_db_ids.append(new_db_id)

    # Record all onboarding picks as "like" swipes
    ...
    user_vector = warm_start_vector(artifacts, all_movie_db_ids)
